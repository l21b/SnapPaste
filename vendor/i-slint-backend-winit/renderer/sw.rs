...
impl super::WinitCompatibleRenderer for WinitSoftwareRenderer {
    fn render(&self, window: &i_slint_core::api::Window) -> Result<DrawOutcome, PlatformError> {
        ...
        // Windows may discard the native pixels during visibility/position changes
        // without resetting softbuffer's age. A partial present can then expose only
        // a caret, hovered control, or damaged rectangle. Repaint the complete frame
        // on every requested draw; this does not schedule continuous rendering.
    ...
    fn resume(
        ...
        // Windows software frames use per-pixel layered presentation, not DWM
        // glass plus a clipped GDI blit. Do not enable winit's glass extension.
        #[cfg(target_os = "windows")]
        let window_attributes = window_attributes.with_transparent(false);
        let winit_window =
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if let RawWindowHandle::Win32(handle) = winit_window.window_handle().map_err(|e| e.to_string())?.as_raw() {
