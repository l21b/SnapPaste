...
pub fn process_clipboard_change_core() -> Result<bool, String> {
    ...
    if let Ok(text) = ctx.read_text()
        && !text.trim().is_empty()
    {
        return Ok(try_process_text_change(text));
    }

    // 只有当剪贴板里完全没有有效文本时，才去尝试抓取纯图片（例如使用截图工具）
    if ENABLE_IMAGE_RECORDING && let Ok(image) = ctx.read_image() {
        return Ok(try_process_image_change(
            image.width,
            image.height,
            image.bytes,
        ));
...
/// 针对图片变化的细分处理
fn try_process_image_change(width: usize, height: usize, raw: Vec<u8>) -> bool {
    ...
    match build_image_record(width, height, &raw) {
        Ok(record) => persist_record(record),
        Err(error) => {
            eprintln!("[Processor] Image encoding failed: {error}");
            false
        }
    }
}

fn try_process_text_change(text: String) -> bool {
...
fn persist_record(record: ClipboardRecord) -> bool {
